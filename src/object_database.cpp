...
namespace chainbase {
    namespace db {
        ...
        const index &object_database::get_index(uint8_t space_id, uint8_t type_id) const {
            assert(_index.size() > space_id);//, "", ("space_id", space_id)("type_id", type_id)("index.size", _index.size()));
        ...
        index &object_database::get_mutable_index(uint8_t space_id, uint8_t type_id) {
            ...
            assert(idx);//, "", ("space", space_id)("type", type_id));
            return *idx;
        }
        ...
        void object_database::wipe(const boost::filesystem::path &data_dir) {
            ...
            boost::filesystem::remove_all(data_dir / "object_database");
            assert(!boost::filesystem::exists(data_dir / "object_database"));
        }
